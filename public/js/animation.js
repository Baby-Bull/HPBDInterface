...
const config = {
    ...
    nickname: "Linhâm ❤️", // nickname(optional)
...
const light = document.querySelector(".switch-aud"),
    ...
    bedroomAud = document.querySelector(".bedroom-aud");
coffeeAud = document.querySelector(".coffee-aud");

//  readMsg() displays the paras in each scene successively. It takes an array of the para elements as input.

const readMsg = (text, indexTime) => {
    ...
    for (let i = 0; i < text.length; i++) {  // this loop goes through all the text msg paras
        setTimeout(() => {  // A timeout of 5s ia applied to all text elements so that appear successively one after the other
            if (i === text.length - 1) text[i].classList.add("readLast");    // this adds a fadeIn-fadeOut animation to elements   
            else text[i].classList.add("read");    // this adds a fadeIn-fadeOut animation to elements   
...
CTAtext.innerHTML = "Bật đèn lên nào !";
...
button.addEventListener("click", function () {
    ...
    if (button.classList.contains("switch")) {
        ...
        */
        ...
        light.play();
        transition(darkroom);
        CTAtext.innerHTML = "Okay, đi nào !";
        setTimeout(function () {
            bedroomAud.play();
            bedroomAud.loop = true;
            button.classList.add("door-out");
            button.classList.remove("switch");
            darkroom.style.display = "none";
            readMsg(roomText, 1.2);
        }, 4000 * tempIndex);
    }
